...
├── Scenes/                   # Generated by Editor tools (not in repo)
├── Scripts/
│   ├── Common/
...
│       └── Editor/           # Editor tools for scene and catalog generation
├── UI/
│   ├── UXML/                 # UI Toolkit layout files
│   │   └── Components/       # Reusable UXML templates
│   └── USS/                  # UI Toolkit style files
│       └── Components/       # Component-specific styles
└── Resources/ShopData/       # Generated by Editor tools (not in repo)
```
...
3. **Wait** for Unity to finish importing all assets and compiling scripts (no errors should appear)
4. Run **Tools > Shop > Create Default Catalog** — generates all ScriptableObject data assets
5. Run **Tools > Shop > Setup Shop Scene** — creates the scene, configures all GameObjects, saves it, and adds it to Build Settings automatically
6. **Enter Play Mode** to interact with the shop UI

> **Important**: The scene and ScriptableObject data are **not** included in the repository because Unity generates unique GUIDs per-project for script references. The Editor tools (steps 4 and 5) handle everything automatically — no manual configuration needed.
